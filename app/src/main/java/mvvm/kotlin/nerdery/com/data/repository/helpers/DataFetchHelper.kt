package mvvm.kotlin.nerdery.com.data.repository.helpers
...
import mvvm.kotlin.nerdery.com.constants.CacheKey
...
import mvvm.kotlin.nerdery.com.data.repository.helpers.DataFetchHelper.DataFetchStyle
import mvvm.kotlin.nerdery.com.data.repository.helpers.DataFetchHelper.DataFetchStyle.*
import mvvm.kotlin.nerdery.com.util.RepositoryUtil.isCacheStale
import mvvm.kotlin.nerdery.com.util.RepositoryUtil.resetCache
import mvvm.kotlin.nerdery.com.util.onMainThread
...
abstract class DataFetchHelper<T>(
    ...
    enum class DataFetchStyle {
         ...
         *
         * Pros: Virtually instantaneous, mostly fresh
         * Cons: Maximizing network calls, requires "two" refreshes to get actual fresh data
         */
        LOCAL_FIRST_NETWORK_REFRESH_ALWAYS,
    ...
    /**
     * Receive a deffered value, guarantees to not be on main thread
    ...
    private suspend fun fetchDataByStyle(
        ...
        when (dataFetchStyle) {
            ...
            LOCAL_FIRST_UNTIL_STALE -> {
                ...
                if (isCacheStale(sharedPreferences, cacheKey, cacheDescriptor, cacheLengthSeconds)) {
                    log("Cache is stale")
                    ...
                    if (resource.data == null) {
                        ...
                        resetCache(sharedPreferences, cacheKey, cacheDescriptor)
                    }
                } else {
